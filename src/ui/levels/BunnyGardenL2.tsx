...
function MovingObstacle({ x, y }: { x: number; y: number }) {
...
function BunnyGardenL2Content({ level }: BunnyGardenL2Props) {
  return (
    <>
      ...
      {(level.movingObstacles || []).map((mov, idx) => (
        <MovingObstacle key={`mov-${idx}`} x={mov.x} y={mov.y} />
      ))}
...
export function BunnyGardenL2({ level }: BunnyGardenL2Props) {
  return (
    <Canvas camera={{ position: [15, 20, 15], fov: 50 }}>
      <BunnyGardenL2Content level={level} />
    </Canvas>
  );
}

export { BunnyGardenL2Content as BunnyGardenL2Scene };
